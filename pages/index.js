...
class Voucher extends React.Component{
  ...
  isActive = store.get('scripttags') && store.get('scripttags').length ? true : false;
    ...
    return(
      <Page>
        <Layout>
              ...
              {(handleMutation, {error, data} ) => {
                ...
                if(data) {
                  ...
                  if(data.scriptTagCreate){
                    ...
                    console.log(!createdTags.find( (element) => {return element.id == data.scriptTagCreate.scriptTag.id } ));

                    if(!createdTags.find( (element) => {return element.id == data.scriptTagCreate.scriptTag.id } )){
                      createdTags.push(
          ...
          <Layout.AnnotatedSection
            title="ガチャ対象クーポン"
            description="クーポンガチャで当たるクーポンコードを入力してください。確率は50%です。※現在機能開発中のため、保存しても表示される抽選対象クーポンコードは「TEST-VOUCHER」のみです。"
          >
